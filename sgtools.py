...
def load_driver() -> WebDriver:
    ...
    # this is used for libby to workaround its bot measures
    options = Options()
    options.headless = True
    return webdriver.Firefox(service=Service(GeckoDriverManager().install()), options=options)

def load_page(driver: WebDriver, url, awaited_element, timeout: int = 10, retries: int = 5) -> str:
    # uses Selenium to load the page, wait until the awaited element shows up, and return the page source
    ...
    while True:
        try:
            ...
            time.sleep(random.random() * 2 + 3)  # give the server a break
            break
        except TimeoutException:
            if retried >= retries:
                print(f"\nCould not find books at {url}\n")
                raise
...
def get_tbr_unlimited(path):
    ...
    for tbri in chunkedtbr:
        with concurrent.futures.ThreadPoolExecutor() as executor:
            ASINUrls = [executor.submit(get_ku_availability,item) for item in tbri]
...
def get_lib_availability(driver,item,location) -> list: # returns LIST of matches (for multiple format availability)
    ...
    matches = response.find_all("div",{"class":"title-tile-shell"})
    output = []
    try:
        for x in matches:
            title = x.find("a",{"class":"title-tile-action halo"})
            status = x.find("div",{"class":"title-status"})
            label =  title['aria-label'].lower().replace(' ','')
            if item[0].lower().replace(' ','') in label and item[1].lower().replace(' ','') in label:
                if 'not owned' in status['aria-label'].lower():
                    pass
                else:
                    print(f'{title['aria-label']} \t https://libbyapp.com{title['href']} \t {status['aria-label']}')
                    output.append(f'{title['aria-label']} \t https://libbyapp.com{title['href']} \t {status['aria-label']}' )
    except:
        return([])
    return(output)

def lib_chunk(chunk,location) -> list:
    print(f'attempting the following chunk : {chunk}')
    driver = load_driver()
    available = []
    for item in chunk:
        available = available + get_lib_availability(driver,item,location)
    driver.quit()
    return available

def libby_main(path = "",location = ""):
    active = True
    if path == "":
        while active: # get path
            path = input("Enter path to Storygraph export\n")
    ...
    if location == "":
        print("Please enter your libby location (e.g MiltonKeynesUK)")
        location = input("\tGo to libby and check what the URL is for your location to find this value\n").replace(' ','').lower()
    # want disticntion between author and title for usage with libby (slightly annoying but whatever)
    with open(path,encoding="utf8") as file:
        tbr = []
        reader = csv.DictReader(file)
        for row in reader:
